...
class createExampleListCls:
    def __init__(self):
        self.dataset = "carObj1" #TODO
        self.split = "test" # TODO
...
class createTfRecordsCls:
    ...
    def __init__(self):
        ...
        self.examples_filename = "carObj1_test_examples.txt" # TODO
        self.dataset = "carObj1" # TODO
        self.split = "test" # TODO
        self.split_type = "primesense" # TODO
        self.add_gt=True
        self.shuffle = False
        self.rgb_format = "png"
...
class eposInfer:
    def __init__(self, objId):
        ...
        self.infer_tfrecord_names = ['carObj1_test'] # TODO
        self.infer_max_height_before_crop = 720 # TODO
        self.infer_crop_size = [1280, 720] # TODO
...
class cameraInput:
    ...
    def callbackRGBImage(self, data):
        ...
        if self.enableInputCallbacks == True:
            try:
                ...
                path = '/home/lele/Codes/epos/datasets/carObj1/test_primesense' # TODO
                # Clean up image folders
                files1 = glob.glob(path + '/000001/rgb/*.png', recursive=True)
                files2 = glob.glob(path + '/000002/rgb/*.png', recursive=True)
                for f in files1:
                    try:
                        os.remove(f)
                    except OSError as e:
                ...
                if self.objectId == 1:
                    self.img1Count += 1
                    #self.ImageReady = False
                    imageio.imwrite(path + '/000001/rgb/000000.png', im_rgb)
                    while not self.ImageReady:
                        imgTest1 = cv2.imread(
                            path + '/000001/rgb/000000.png')
                        if imgTest1.size != 0:
                            self.ImageReady = True
                elif self.objectId == 2:
                    self.img2Count += 1
                    imageio.imwrite(path + '/000002/rgb/000000.png',  im_rgb)
                    while not self.ImageReady:
                        imgTest2 = cv2.imread(
                            path + '/000002/rgb/000000.png')
                        if imgTest2.size != 0:
                            self.ImageReady = True
    ...
    def get_pose(self, req):
        ...
        #while self.rgb is None or not self.ImageReady: # run my bags from 202207
